...
def run_benders(
    ...
    """
    timer_start = time()
    mp_instance = create_master_problem_instance(paths, empire_config, periods=periods_active, scenarios=operational_input_params.scenarios)
    ...
    # solve_master_problem(mp_instance, empire_config.optimization_solver, flags, paths, empire_config.temporary_directory, save_flag=False)
    ...
    for iteration in range(empire_config.max_benders_iterations):
        ...
        if empire_config.parallel_benders_flag:
            with ThreadPoolExecutor(max_workers=empire_config.n_cores) as executor:  # adjust number of workers
                futures = [
                    executor.submit(solve_and_create_cut, sp_instance, capacity_params, period_active, scenario, empire_config, paths, mp_instance)
            ...
            for (period_active, scenario), sp_instance in subproblems.items():
                cut = solve_and_create_cut(sp_instance, capacity_params, period_active, scenario, empire_config, paths, mp_instance)
                cuts.append(cut)
        for cut in cuts:
            mp_instance.cut_constraints.add(expr=cut)

        mp_objective = solve_master_problem(mp_instance, empire_config, paths, save_flag=False)
        capacity_params = extract_capacity_params(mp_instance)
...
def solve_and_create_cut(sp_instance, capacity_params, period_active, scenario, empire_config, paths, mp_instance):
