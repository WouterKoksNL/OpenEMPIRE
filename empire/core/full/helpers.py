...
def log_problem_statistics(instance, logger):
    ...
    logger.info("LengthRegSeason: %s", value(instance.length_reg_season))
    logger.info("PeakSeasons: %s", len(instance.FirstHoursOfPeakSeason))
    logger.info("LengthPeakSeason: %s", value(instance.length_peak_season))
    logger.info("")
    logger.info("Discount rate: %s", value(instance.discount_rate))
