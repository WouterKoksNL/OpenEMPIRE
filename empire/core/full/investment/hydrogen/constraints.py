...
def define_hydrogen_investment_constraints(model, 
    ):
    """Define constraints for hydrogen infrastructure investment.
    
    Args:
        model: Pyomo abstract model
        model.leap_years_investment: Number of years per investment period
        model.repurposeEnergyFlowFactor: Repurpose energy flow factor
    """
    ...
    def lifetime_rule_pipeline(model, n1, n2, i):
        ...
        startPeriod = 1
        if value(1 + i - model.hydrogenPipelineLifetime / model.leap_years_investment) > startPeriod:
            startPeriod = value(1 + i - model.hydrogenPipelineLifetime / model.leap_years_investment)
        ...
        if (n1, n2) in model.RepurposeDirectionalLinks and (n2, n1) in model.RepurposeDirectionalLinks:
            return sum((model.hydrogenPipelineBuilt[n1, n2, j] + 
                       model.repurposedPipelineBuilt[n1, n2, j] * ng_h2_conversion_factor * model.repurposeEnergyFlowFactor +
                       model.repurposedPipelineBuilt[n2, n1, j] * ng_h2_conversion_factor * model.repurposeEnergyFlowFactor) 
    ...
    def lifetime_rule_elyzer(model, n, i):
        ...
        startPeriod = 1
        if value(1 + i - model.elyzerLifetime / model.leap_years_investment) > startPeriod:
            startPeriod = value(1 + i - model.elyzerLifetime / model.leap_years_investment)
    ...
    def lifetime_rule_reformer(model, n, p, i):
        ...
        startPeriod = 1
        if value(1 + i - model.ReformerPlantLifetime[p] / model.leap_years_investment) > startPeriod:
            startPeriod = value(1 + i - model.ReformerPlantLifetime[p] / model.leap_years_investment)
    ...
    def hydrogen_storage_lifetime_rule(model, n, b, i):
        ...
        startPeriod = 1
        if value(1 + i - model.hydrogenStorageLifetime[b] / model.leap_years_investment) > startPeriod:
            startPeriod = value(1 + i - model.hydrogenStorageLifetime[b] / model.leap_years_investment)
    ...
    def lifetime_rule_H2import(model, n, t, i):
        ...
        startPeriod = 1
        if value(1 + i - model.H2TerminalLifetime[t] / model.leap_years_investment) > startPeriod:
            startPeriod = value(1 + i - model.H2TerminalLifetime[t] / model.leap_years_investment)
    ...
    def co2_pipeline_lifetime_rule(model, n1, n2, i):
        ...
        startPeriod = 1
        if value(1 + i - model.CO2PipelineLifetime / model.leap_years_investment) > startPeriod:
            startPeriod = value(1 + i - model.CO2PipelineLifetime / model.leap_years_investment)
