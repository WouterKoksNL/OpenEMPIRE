...
def define_industry_investment_expressions(model, steel_CCS_cost_increase):
    ...
    def prepIndustryInvCost_rule(model):
        ...
        for i in model.Period:
            for p in model.SteelPlants:
                ...
                costperperiod = costperyear * (1 - (1 + model.discount_rate) ** 
                               -(min(value((len(model.Period) - i + 1) * model.leap_years_investment), 
                                    model.steelPlantLifetime[p]))) / (1 - (1 / (1 + model.discount_rate)))
        ...
        for i in model.Period:
            for p in model.CementPlants:
                ...
                costperperiod = costperyear * (1 - (1 + model.discount_rate) ** 
                               -(min(value((len(model.Period) - i + 1) * model.leap_years_investment), 
                                    model.cementPlantLifetime[p]))) / (1 - (1 / (1 + model.discount_rate)))
        ...
        for i in model.Period:
            for p in model.AmmoniaPlants:
                ...
                costperperiod = costperyear * (1 - (1 + model.discount_rate) ** 
                               -(min(value((len(model.Period) - i + 1) * model.leap_years_investment), 
                                    model.ammoniaPlantLifetime[p]))) / (1 - (1 / (1 + model.discount_rate)))
