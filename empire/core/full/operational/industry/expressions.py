...
def define_industry_operational_expressions(model, EMISSION_CAP, steel_CCS_capture_rate=None):
    ...
    def steel_opex_rule(model, i, w, gp):
        ...
        for p in model.SteelPlants:
            steel_opex += sum(model.seasScale[s] *  (model.genFuelCost['Coal',i] * model.steel_coalConsumption[p,i] +
                                                    model.genFuelCost['Oil_existing',i] * model.steel_oilConsumption[p,i] +
                                                    model.genFuelCost['Bio_existing',i] * model.steel_bioConsumption[p,i] +
        ...
        for p in model.CementPlants:
            if "ng" in p.lower():
                if EMISSION_CAP is False:
                    cement_opex += sum(model.seasScale[s] * model.CO2price[i] * model.genCO2TypeFactor['Gas_existing'] * Constants.GJperMWh * Constants.ng_MWhPerTon / 1000 * model.cement_fuelConsumption[p,i] * model.cementProduced[n,p,h,i,w,gp] for n in model.CementProducers for (s,h) in model.HoursOfSeason)
    ...
    def ammonia_opex_rule(model, i, w, gp):
        ...
        for p in model.AmmoniaPlants:
            if "ng" in p.lower():
                if EMISSION_CAP is False:
                    ammonia_opex += sum(model.seasScale[s] * model.CO2price[i] * model.genCO2TypeFactor['Gas_existing'] * Constants.GJperMWh * Constants.ng_MWhPerTon / 1000 * model.ammonia_fuelConsumption[p] * model.ammoniaProduced[n,p,h,i,w,gp] for n in model.AmmoniaProducers for (s,h) in model.HoursOfSeason)
    ...
    #     oil_emission_factor = model.genCO2TypeFactor['Oil_existing'] * GJperMWh
    ...
    #     transport_opex = 0
    #     transport_opex += sum(model.seasScale[s] * model.sceProbab[w] * model.genFuelCost['Oil_existing',i] * model.transport_energyConsumption[v,i] * GJperMWh * model.transportDemandMet[n,v,h,i,w,gp] for n in model.OnshoreNode for (s,h) in model.HoursOfSeason for w in model.Scenario for v in model.VehicleTypes if ('kerosene' in v.lower() or 'diesel' in v.lower() or 'gasoline' in v.lower()))
    ...
    #     if EMISSION_CAP is False:
    #         transport_opex += sum(model.seasScale[s] * model.sceProbab[w] * model.genCO2TypeFactor['Oil_existing'] * GJperMWh * model.transportDemandMet[n,v,h,i,w,gp] for n in model.OnshoreNode for (s,h) in model.HoursOfSeason for w in model.Scenario for v in model.VehicleTypes if ('kerosene' in v.lower() or 'diesel' in v.lower() or 'gasoline' in v.lower()))
