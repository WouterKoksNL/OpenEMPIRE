...
def write_results(
                        ...
                        for gp in instance.GasScenario:
                            for (s,h) in instance.HoursOfSeason:
                                my_string=[
                                    ...
                                    s,
                                    h, 
                                    value(sum(instance.genOperational[n,g,h,i,w, gp] for g in instance.Generator if (n,g) in instance.GeneratorsOfNode)), 
                                ...
                                my_string.extend([value(sum(-instance.storCharge[n,b,h,i,w] for b in instance.Storage if (n,b) in instance.StoragesOfNode)), 
                                    ...
                                    value(sum(instance.genOperational[n,g,h,i,w, gp]*instance.genCO2TypeFactor[g]*(3.6/instance.genEfficiency[g,i]) for g in instance.Generator if (n,g) in instance.GeneratorsOfNode)/sum(instance.genOperational[n,g,h,i,w, gp] for g in instance.Generator if (n,g) in instance.GeneratorsOfNode) if value(sum(instance.genOperational[n,g,h,i,w, gp] for g in instance.Generator if (n,g) in instance.GeneratorsOfNode)) != 0 else 0)])
                                writer.writerow(my_string)
                                    ...
                                    for gp in instance.GasScenario:
                                        for (s,h) in instance.HoursOfSeason:
                                            writer.writerow([
                                                ...
                                                h,
                                                g,
                                                value(instance.sceProbab[w]*instance.seasScale[s]*(instance.genCapAvail[n,g,h,i,w]*instance.genInstalledCap[n,g,i] - instance.genOperational[n,g,h,i,w, gp]))
                                            ])
                    ...
                    if (t,g) in instance.GeneratorsOfTechnology: 
                        for i in instance.Period:
                            writer.writerow([
                                ...
                                g,
                                inv_per[int(i-1)], 
                                value(sum(instance.sceProbab[w]*instance.seasScale[s]*(instance.genCapAvail[n,g,h,i,w]*instance.genInstalledCap[n,g,i] - instance.genOperational[n,g,h,i,w,gp])/1000 for w in instance.Scenario for gp in instance.GasScenario for (s,h) in instance.HoursOfSeason))
                            ])
    ...
    with open(result_file_path / 'results_output_EuropePlot.csv', 'w', newline='') as f:
        ...
        for i in instance.Period:
            my_string=[inv_per[int(i-1)]]
            for g in instance.Generator:
                my_string.append(value(sum(instance.sceProbab[w]*instance.seasScale[s]*instance.genOperational[n,g,h,i,w,gp]/1000 for n in instance.Node if (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason for w in instance.Scenario for gp in instance.GasScenario)))
            writer.writerow(my_string)
        writer.writerow([""])
    ...
    with open(result_file_path / 'results_output_EuropeSummary.csv', 'w', newline='') as f:
        ...
        for i in instance.Period:
            for w in instance.Scenario:
                for gp in instance.GasScenario:
                    ...
                    value(sum(instance.seasScale[s]*instance.genOperational[n,g,h,i,w,gp]*instance.genCO2TypeFactor[g]*(3.6/instance.genEfficiency[g,i]) for (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason))]
                    ...
                    my_string.extend([value(sum(instance.seasScale[s]*instance.genOperational[n,g,h,i,w,gp]/1000 for (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason)), 
                    value(sum(instance.seasScale[s]*instance.genOperational[n,g,h,i,w,gp]*instance.genCO2TypeFactor[g]*(3.6/instance.genEfficiency[g,i]) for (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason)/sum(instance.seasScale[s]*instance.genOperational[n,g,h,i,w,gp] for (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason)), 
                    ...
                    value(sum(instance.seasScale[s]*(instance.genCapAvail[n,g,h,i,w]*instance.genInstalledCap[n,g,i] - instance.genOperational[n,g,h,i,w,gp])/1000 for (n,g) in instance.GeneratorsOfNode if g == 'Hydrorun-of-the-river' or g == 'Windonshore' or g == 'Windoffshore' or g == 'Solar' for (s,h) in instance.HoursOfSeason)), 
        ...
        for g in instance.Generator:
            for i in instance.Period:
                ...
                value(sum(instance.seasScale[s]*instance.sceProbab[w]*instance.genOperational[n,g,h,i,w,gp]/1000 for n in instance.Node if (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason for w in instance.Scenario for gp in instance.GasScenario))])
        writer.writerow([""])
    ...
    if IAMC_PRINT:
        ...
        f = pd.DataFrame(columns=["model", "scenario", "region", "variable", "unit", "subannual"]+[value(2020+(i)*instance.leap_years_investment) for i in instance.Period])
        ...
        def row_write(df, region, variable, unit, subannual, input_value, scenario=Scenario, modelname=Modelname):
            df2 = pd.DataFrame([[modelname, scenario, region, variable, unit, subannual]+input_value],
                               columns=["model", "scenario", "region", "variable", "unit", "subannual"]+[value(2020+(i)*instance.leap_years_investment) for i in instance.Period])
            df = pd.concat([df, df2], ignore_index=True)
            return df

        f = row_write(f, "Europe", "Discount rate|Electricity", "%", "Year", [value(instance.discount_rate*100)]*len(instance.Period)) #Discount rate
        ...
        f = row_write(f, "Europe", "Investment|Energy Supply|Electricity|Transmission and Distribution", "billion US$2010/yr", "Year", [value((1/instance.leap_years_investment)*USD10perEUR18* \
                    sum(instance.transmissionInvCost[n1,n2,i]*instance.transmissionInvCap[n1,n2,i] for (n1,n2) in instance.BidirectionalArc)) for i in instance.Period]) #Total European transmission investment cost
        for w in instance.Scenario:
            for gp in instance.GasScenario:
                f = row_write(f, "Europe", "Emissions|CO2|Energy|Supply|Electricity", "Mt CO2/yr", "Year", [value(Mtonperton*sum(instance.seasScale[s]*instance.genCO2TypeFactor[g]*(GJperMWh/instance.genEfficiency[g,i])* \
                        instance.genOperational[n,g,h,i,w,gp] for (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason)) for i in instance.Period], Scenario+"|"+str(w)) #Total European emissions per scenario
                f = row_write(f, "Europe", "Secondary Energy|Electricity", "EJ/yr", "Year", \
                        [value(sum(EJperMWh*instance.seasScale[s]*instance.genOperational[n,g,h,i,w,gp] for (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason)) for i in instance.Period], Scenario+"|"+str(w)) #Total European generation per scenario
                for g in instance.Generator:
                    f = row_write(f, "Europe", "Active Power|Electricity|"+dict_generators[str(g)], "MWh", "Year", \
                        [value(sum(instance.seasScale[s]*instance.genOperational[n,g,h,i,w,gp] for n in instance.Node if (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason)) for i in instance.Period], Scenario+"|"+str(w)) #Total generation per type and scenario
                for (s,h) in instance.HoursOfSeason:
                    for n in instance.Node:
                        f = row_write(f, dict_countries_reversed[str(n)], "Price|Secondary Energy|Electricity", "US$2010/GJ", seasonhours[h-1], \
                            [value(instance.dual[instance.FlowBalance[n,h,i,w]]/(GJperMWh*instance.operationalDiscountrate*instance.seasScale[s]*instance.sceProbab[w])) for i in instance.Period], Scenario+"|"+str(w)+str(s))
        for g in instance.Generator:
            ...
            if value(instance.genMargCost[g,instance.Period[1]]) != 0: 
