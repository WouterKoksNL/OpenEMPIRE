...
def filter_by_column(
    df: pd.DataFrame, 
    col_name: str | tuple[str], 
    values: None | list | int | pd.DataFrame,
    ):
    """Filter dataframe by values in specified column.

    Args:
        df (pd.DataFrame): _description_
        col_name (str | tuple[str]): _description_
    ...
    if isinstance(col_name, tuple):
        if not isinstance(values, pd.DataFrame):
            raise ValueError("When filtering by multiple columns, values must be provided as a DataFrame.")
        # keep rows for which all columns match the corresponding values
        mask = pd.MultiIndex.from_frame(df[values.columns]).isin(pd.MultiIndex.from_frame(values))
        df = df[mask]
        return df
        # In this case we filter by multiple columns, selecting only the entries that are in all specified columns
