...
def load_sets(data, model, input_data_dir, set_name_list, component="Sets"):
...
def load_params(data, model, input_data_dir, component, param_name_list):
...
def load_data_from_files(data: DataPortal, model: AbstractModel, input_data_dir: Path, inputs: dict[str, list[str]]) -> None:
    ...
    for component, file_list in inputs.items():
        if component == "Sets":
            load_sets(data, model, input_data_dir, component, file_list)
        else:
            load_params(data, model, input_data_dir, component, file_list)
